...
$$
S(E, L, R) = B_R - B _ {L-1} = N
$$
と言い換えられます．$M$ 以下の各 $R$ に対し，$B _ {L-1} = B_{R} - N$ なる $L$ は $\Theta(1)$ 時間で検索できるため，合わせて $\Theta(M) = \Theta(N^{1/E})$ 時間で解くことができます．
...
\end{aligned}
$$
より $1 \le W \le \lfloor (3 N)^{1/3} \rfloor$ の範囲を調べればよいです．よって $E = 2$ の場合を $\Theta( N^{1/3} \log N )$ 時間で解くことができます．
