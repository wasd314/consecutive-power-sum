...
2つ目については，Miller–Rabin 素数判定法を用いることにします．Miller–Rabin 素数判定法は一般には確率的なアルゴリズムですが，$n$ の上限が分かっている場合にはテストする底の集合をうまく選ぶことで決定的なアルゴリズムとしても使えます．中でも有用な（要素数が小さく上限が大きい）底の集合と上限の組はいくつも知られています（参考: [Wikipedia](https://en.wikipedia.org/wiki/Miller%E2%80%93Rabin_primality_test#Testing_against_small_sets_of_bases), [Deterministic variants of the Miller-Rabin primality test](https://miller-rabin.appspot.com/)）．特に
...
-->
- $n \lt 18\,446\,744\,073\,709\,551\,616 \,(= 2^{64})$ なら $\{\, 2, 325, 9\,375, 28\,178, 450\,775, 9\,780\,504, 1\,795\,265\,022 \,\}$ を，
...
底の集合に取ることで，各々の範囲において決定的に正しく素数判定ができることが知られており（参考: [OEIS](https://oeis.org/A014233)，[論文](https://doi.org/10.1090/mcom/3134)），本問題の制約下では十分です．

なお，上記3集合のうち最初の2行に示したものを保証範囲外の $n$ に用いた実装に対する反例が，テストケースに含まれています．
